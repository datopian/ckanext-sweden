...
def get_weekly_new_dataset_totals():
    '''For each week, return the cumulative total of datasets.'''
    new_datasets = get_new_datasets()

    return _weekly_totals(new_datasets, cumulative=True)


def get_weekly_dataset_activity():
    '''For each week, get the number datasets with some sort of activity.'''
    pkg_revisions = get_package_revisions()

    return _weekly_totals(pkg_revisions)


def get_weekly_dataset_activity_new():
    '''For each week, get the number of new datasets.'''
    new_datasets = get_new_datasets()

    return _weekly_totals(new_datasets)


def _weekly_totals(id_date_list, cumulative=False):
    '''
    ...
    Return a list of tuples in the form (timestamp, id count). The timestamp
    is in milliseconds.
    ...
    ids_week_start = [(pkg_id, _datetime_to_timestamp(
                               _transform_to_week_start(date_time)))
